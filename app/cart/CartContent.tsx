...
export default function CartContent() {
  ...
  if (cart.length === 0) {
    return (
      <div className="flex flex-col items-center gap-5 montserrat mt-12 sm:mt-24 px-4">
        <p className="text-center">Добавьте что-нибудь в корзину</p>
        <Link
          href="/search"
          className="bg-neutral-800 text-white px-8 sm:px-12 py-2 rounded-full text-sm sm:text-base"
        >
          Каталог
        </Link>
      </div>
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex flex-col items-center pb-6 sm:pb-10 montserrat gap-4 sm:gap-5 mt-4 sm:mt-6">
        ...
        <Button onClick={handleCheckout} variant="outline" size="long">
          Оформить заказ
        </Button>
      </div>
    </div>

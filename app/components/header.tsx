...
export default function Header() {
  ...
  return (
    <header className="flex justify-around items-center w-full mt-4">
          ...
          >
            ...
            {isClient && (
              <ul className={`absolute left-0 mt-6 flex flex-col gap-4 bg-white shadow-lg border border-neutral-100 p-4 rounded-3xl overflow-hidden transition-all duration-300 ease-in-out origin-top min-w-[160px] z-50
                ...
                <li>
                  <Link href='/contact' className="block px-4 py-2 text-neutral-800 hover:bg-neutral-100 rounded-3xl transition-colors duration-200">
                    Обратная связь
                  </Link>
                </li>
              </ul>
      ...
      <nav className='lg:static fixed bottom-0 max-[1024px]:w-full flex justify-center bg-white z-50'>
        <ul className={`flex gap-6 max-[1024px]:py-6 max-[1024px]:gap-24
          max-[430px]:gap-16
          ${isMobileMenuOpen ? 'hidden' : ''}`}>
          <li>
            <Link href='/wishlist' className="relative z-50">
              <Heart className="relative z-50" />
            </Link>
          </li>

          <li>
            <Link href='/search' className="relative z-50">
          ...
          <li>
            <Link href='/cart' className="relative z-50">
              <ShoppingCart className="relative z-50" />
            </Link>
          </li>
        </ul>
      </nav>

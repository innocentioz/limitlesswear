  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 mt-6 sm:mt-8 lg:mt-12 mb-24 sm:mb-32">
      <h1 className="text-2xl sm:text-2xl font-bold mb-4">
        {categoryNames[category] || "Категория не найдена"}
      </h1>
      ...
      <div className="flex flex-col gap-3 items-start w-full">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 w-full">
          <div className="text-base sm:text-base montserrat">
            ...
            <Link href={`/category/${category}`} className="text-neutral-500 hover:text-neutral-900">
              <span> {categoryNames[category] || "Категория не найдена"}</span>
            </Link>
          </div>
      ...
      <div className="flex justify-center items-center mt-8 sm:mt-12 space-x-2 gap-4 sm:gap-20">
        ...
        <div className="flex gap-6 sm:gap-10 text-2xl sm:text-2xl">
          {getPageNumbers().map((pageNumber) => (
            <button
              key={pageNumber}
              className={`${pageNumber === page ? "text-neutral-800" : "text-neutral-400"} p-2`}
